...
public class TripHome {
	...
	private static final Log log = LogFactory.getLog(TripHome.class);
	...
	public Trip merge(Trip detachedInstance) {
		...
		try {
			Trip result = (Trip) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Trip findById(java.lang.Integer id) {
		log.debug("getting Trip instance with id: " + id);
		try {
			Trip instance = (Trip) sessionFactory.getCurrentSession().get(
					"daos.Trip", id);
			if (instance == null) {
	...
	public List findByExample(Trip instance) {
		log.debug("finding Trip instance by example");
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("daos.Trip").add(Example.create(instance))
					.list();
