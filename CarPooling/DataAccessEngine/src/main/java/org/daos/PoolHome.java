...
import org.pojos.Pool;

/**
 * Home object for domain model class Pool.
 * @see daos.Pool
 * @author Hibernate Tools
 */
public class PoolHome {

	private static final Log log = LogFactory.getLog(PoolHome.class);
	...
	public void persist(Pool transientInstance) {
		log.debug("persisting Pool instance");
		try {
	...
	public Pool merge(Pool detachedInstance) {
		...
		try {
			Pool result = (Pool) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Pool findById(java.lang.Integer id) {
		...
		try {
			Pool instance = (Pool) sessionFactory.getCurrentSession().get(
					"daos.Pool", id);
			if (instance == null) {
	...
	public List findByExample(Pool instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("daos.Pool").add(Example.create(instance))
					.list();
